...
function App() {
  ...
  // Función para obtener URL inicial
  const getIconUrl = (fileName) => {
    if (!fileName) return null;
    return `/pokemon-icons/${String(fileName).toUpperCase()}.png`; 
  };
        ...
        {data.team?.map((pkmn, pIndex) => (
          <div key={pIndex} style={styles.card}>
            <div style={{display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px'}}>
               ...
               {/* 🖼️ ICONO INTELIGENTE (ESTA ES LA SOLUCIÓN) 🖼️ */}
               <img 
                 ...
                 onError={(e) => { 
                    // AQUÍ ESTÁ LA MAGIA:
                    // Si falla QUILAVA_1.png, calcula el nombre base QUILAVA.png
                    const baseName = String(pkmn.species).toUpperCase();
                    const backupUrl = `/pokemon-icons/${baseName}.png`;
                    
                    // Si aún no hemos probado el backup, lo probamos ahora
                    if (!e.target.src.toUpperCase().endsWith(backupUrl.toUpperCase())) {
                        console.log(`⚠️ Usando icono base para: ${pkmn.species}`);
                        e.target.src = backupUrl; 
                    } else {
                        // Si falla el backup, se oculta
